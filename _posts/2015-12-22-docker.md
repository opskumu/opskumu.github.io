...
![docker 桥接图](images/docker_bridge.png)
...
```
$ docker inspect --format="{% raw %}{{.Volumes}}{% endraw %}" data
...
``` bash
$ sudo docker run -t -i -d -v /test --name test centos:6.4 echo hello
```
...
``` bash
$ sudo docker run --rm --volumes-from test -v $(pwd):/backup centos:6.4 tar cvf /backup/test.tar /test
...
``` bash
$ sudo docker run -t -i -d -v /test --name test4 centos:6.4  /bin/bash
$ sudo docker run --rm --volumes-from test4 -v $(pwd):/backup centos:6.4 tar xvf /backup/test.tar -C /
